...
| Phase | 内容 | 成果 | 目安 |
|---|---|---|---|
| **1** | `ingest.js` 共通化 + `/api/health/activity` `/workout` `/sync` + `ActivityCard`（睡眠表示を含む） | **「連携したのに見えない」が解消される**。歩数・運動・睡眠がダッシュボードに出る | 2〜3日 |
...
1. **ショートカットでワークアウト（種目・開始終了時刻）が取れるか** — 実機で 1 度確認する。
   取れない場合、`workouts` コレクションは Phase 5（ネイティブ）まで空のまま、
   運動判定は `exerciseMinutes` / `activeEnergy` の日次値のみで行う。
2. **同期の実行回数** — 1日1回（12時）か、2回（12時・23時）か。歩数の当日精度に影響する。
3. **`activityCalorieMode` の既定** — 本設計では `display`（表示のみ）を推奨。
