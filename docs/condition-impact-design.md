...
```js
{
  bedtime: '24:00',        // 就寝予定時刻（sleep 軸に必須）
  wakeTime: '07:00',
  conditionEnabled: true,  // 機能のオン/オフ
  conditionCheckIn: { morning: true, evening: true },  // 問いかけの許可
}
```

`DietPlanWizard.js` に Step 4（任意・スキップ可）として追加、または設定モーダルから編集。
...
- `gemini-client.js`: `EXTENDED_MACROS_PROPERTIES` に Tier1 10フィールド追加 + 各プロンプトに null ルール明記
- `page.js`: `getDailyTotals` に新フィールド合算
- `firestore.js`: `conditionLogs` の CRUD ヘルパー（`saveConditionLog` / `getConditionLog` / `getConditionLogs`）
- `api/health/sleep/route.js`: HealthKit 睡眠受信 API
- プロフィールに `bedtime` / `wakeTime` 追加
- `ConditionCheckIn.js`: 体感入力 UI（この時点で**データを貯め始める**のが重要）
...
- **就寝時刻の扱い**: プロフィール固定値でよいか、それとも曜日別に設定したいか（平日/休日で大きく違う場合、`late_meal` の精度が変わる）
- **カフェイン/アルコールの推定精度**: 「コーヒー」だけで mg を当てるのは難しい。よく飲むものを**マイドリンク登録**して固定値を持つ方式にするか
- **体感入力のタイミング**: 集中力を「夕方に1回」で足りるか、「午前/午後の2回」にするか（精度 vs 摩擦のトレードオフ）
- **Phase 1 のリリース単位**: スコアが出ない Phase 1 を単体でリリースするか、Phase 2 まで作ってから出すか
