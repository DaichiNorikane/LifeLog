...
- **エレナのコメント生成タイミング**: 食事追加のたびに呼ぶとコストが嵩む。日1回か、モーダルを開いた時か
- **evening-preview で伝える内容**: 睡眠予測が低い日に何を提案するか（カフェイン・遅い食事のどちらを優先して指摘するか）
- **重み係数のチューニング**: 実データが数日溜まった時点で、体感と乖離している軸がないか確認する
