...
export const searchRecipesWithGemini = async (query) => {
    const prompt = `
        ...
        - その代わり、ユーザーがGoogleやクックパッド等で検索するための「最適な検索キーワード（sourceQuery）」を提供してください。
        - **重要: sourceQuery は検索ヒット率を最大化するため、極力シンプルにしてください。**
            - NG: "春キャベツと豚バラ肉の回鍋肉風ピリ辛味噌炒め"
            - OK: "キャベツ 豚肉 味噌炒め"
            - ルール: 「食材名」と「料理名」の2〜3単語のスペース区切り。修飾語（ふわふわ、絶品、〜風）や助詞は削除すること。
...

        {
            "recipes": [
                {
                    "foodName": "料理名（ここは具体的で魅力的な名前でOK）",
                    "description": "魅力的な短い説明",
                    "ingredients": "（詳細は検索リンク先でご確認ください）",
                    "instructions": [],
                    "calories": 推定カロリー(数値),
                    "macros": { "protein": 数値, "fat": 数値, "carbs": 数値 },
                    "sourceQuery": "検索用キーワード（例: 豚肉 キャベツ 味噌炒め）"
                },
                ...
            ]
        }
