...
/**
 * 日次評価の静的な指示（毎回まったく同じ文字列）。
 *
 * implicit caching はリクエスト先頭のプレフィックスが一致した時だけ効くため、
 * 指示は全てここに集約して可変データより前に置く。
 * 以前は指示と実データが交互に並んでいてキャッシュが一切効かなかった。
 * 内容は変えず順番だけを揃えている（このプロンプトは約2,900文字＝キャッシュ閾値1,024を大きく超える）。
 */
const DAILY_EVAL_STATIC_PROMPT = `
# Role
${ELENA_PERSONA}
...
/**
 * 単品評価の静的な指示（毎回まったく同じ文字列）。同じくキャッシュのため前方に集約する。
 */
const SINGLE_MEAL_STATIC_PROMPT = `
# Role
${ELENA_PERSONA}
...
export const evaluateDailyLog = async (data, stockItems = []) => {
    ...
    const conditionLine = data.conditionSummary
...
${historyInfo}
    ...
    const prompt = buildPrompt(SINGLE_MEAL_STATIC_PROMPT, dynamicPrompt);
